...
net2gridApiModule.service('generalDevicesService', ['$http', 
                                                    ...
                                                    'zigbeeLutService', 
													'utilityService',
                                                    function($http, $rootScope, $log, $timeout, ipAddressService, zigbeeLutService, utilityService) {
...
//	}
	...
	var setupGeneralDevices = function() {
		...
		$http.get('http://'+ip+'/zgd/services')
		.success(function(response) {

			generalDevices = [];

			var d;
			for(d=0; d<response.list.length; d++) {

				generalDevices.push({

					label: d,
					ipZgd: ip,
					eui: response.list[d].addr.eui,
					nwkAddr: response.list[d].addr.nwkAddr,
					endPoints: [],
				...
				var e;
				for(e=0; e<response.list[d].epDescriptorList.length; e++) {

					generalDevices[d].endPoints.push({

						ep: response.list[d].epDescriptorList[e].endpoint,
						deviceId: response.list[d].epDescriptorList[e].deviceId,
						deviceVersion: response.list[d].epDescriptorList[e].deviceVers,
						clusters: []
					});

					var c;
					for(c=0; c<response.list[d].epDescriptorList[e].serverClusters.length; c++) {

						generalDevices[d].endPoints[e].clusters.push({
							
							clusterId: response.list[d].epDescriptorList[e].serverClusters[c],
							attributesIds: [],
							commandIds: []
						});					
													
						/*discAttrReq(generalDevices[d], response.list[d].epDescriptorList[e].serverClusters[c], i, d).then(function(promise) {
							
							setupAttributeIds(promise);		
						});*/

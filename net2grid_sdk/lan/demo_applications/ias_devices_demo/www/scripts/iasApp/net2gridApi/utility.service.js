...
net2gridApiModule.service('utilityService', ['$log', 'zigbeeLutService',function($log, zigbeeLutService) {
	 ...
	 */
	 var euiToHex = function (euiDec) {
        ...
        // Checks each index of each eui and converts to hex
        var euiHex = [];
        var i, e;

        // Loop through eui index
        for (e = 0; e < euiDec.length; e++) {

            // Adds extra zero when eui content < 0x10
            if (euiDec[e].toString(16).length === 2) {

                euiHex.push(euiDec[e].toString(16).toUpperCase());
            }
            else {

                euiHex.push('0' + euiDec[e].toString(16).toUpperCase());
            }
			
			if(e < euiDec.length-1) {
